...
void createMatrix(Graph *graph)
{
    ...
    for (uint8_t i = 0; i < graph->size; i++)
    {
        ...
        {
            if (i == j) graph->FWweight[i][j] = 0;
            else graph->FWweight[i][j] = INFINITY;
        }
    }
    ...
    for (uint8_t i = 0; i < graph->size; i++)
    {
        ...
        for (uint8_t j = 0; j < graph->size; j++)
        {
            if (i == j) graph->FWroutes[i][j] = 0;
            else graph->FWroutes[i][j] = j;
        }
    }
...
void printMatrix(Graph *graph)
{
    ...
    for (int i = 0; i < graph->size; i++)
    {
        ...
        {
            if (matrix01[i][j] == INFINITY) printf("INF ");
            else printf("%3d ", matrix01[i][j]);
        }
        printf("\n");
    }
